export const tarotCards = [
  // Arcanos Mayores
  {
    name: "El Loco",
    image: "/tarot-images-bis/00-TheFool.jpg",
    description: {
      upright:
        "El inicio de un viaje, la fe ciega, la libertad, la inocencia, la espontaneidad. Un nuevo capítulo se abre, sin miedos, con la curiosidad como guía.",
      reversed:
        "La imprudencia, el miedo a dar el salto, la indecisión, la falta de dirección. Te aferras a lo conocido, temiendo lo desconocido.",
    },
  },
  {
    name: "El Mago",
    image: "/tarot-images-bis/01-TheMagician.jpg",
    description: {
      upright:
        "El poder de la manifestación, la habilidad, el ingenio, la confianza en uno mismo. Tienes las herramientas, sabes cómo usarlas y crear tu realidad.",
      reversed:
        "La manipulación, la falta de habilidades, la inseguridad, la duda. No confías en tus capacidades, y el poder se vuelve en tu contra.",
    },
  },
  {
    name: "La Suma Sacerdotisa",
    image: "/tarot-images-bis/02-TheHighPriestess.jpg",
    description: {
      upright:
        "La intuición, la sabiduría interior, los secretos, la paciencia. Escucha tu voz interior, ella te guiará por el camino correcto.",
      reversed:
        "Los secretos revelados, la superficialidad, la desconexión de la intuición. Ignoras la voz de tu alma, y te pierdes en la confusión.",
    },
  },
  {
    name: "La Emperatriz",
    image: "/tarot-images-bis/03-TheEmpress.jpg",
    description: {
      upright:
        "La abundancia, la fertilidad, la creatividad, la conexión con la naturaleza. La vida florece a tu alrededor, y te nutres de ella.",
      reversed:
        "La esterilidad, la falta de creatividad, la desconexión, la negligencia. Te alejas de la vida, y la inspiración se marchita.",
    },
  },
  {
    name: "El Emperador",
    image: "/tarot-images-bis/04-TheEmperor.jpg",
    description: {
      upright:
        "La estructura, el control, la autoridad, la estabilidad. Construyes un imperio sólido, basado en la razón y la disciplina.",
      reversed:
        "La tiranía, la falta de control, la rigidez, la inestabilidad. El poder se corrompe, y la estructura se derrumba.",
    },
  },
  {
    name: "El Hierofante",
    image: "/tarot-images-bis/05-TheHierophant.jpg",
    description: {
      upright:
        "La tradición, la espiritualidad, el consejo, la guía. Buscas respuestas en la sabiduría ancestral, en la comunidad y en la fe.",
      reversed:
        "La rebeldía, la hipocresía, la conformidad, la rigidez. Te alejas de la tradición, pero te pierdes en el caos.",
    },
  },
  {
    name: "Los Enamorados",
    image: "/tarot-images-bis/06-TheLovers.jpg",
    description: {
      upright:
        "El amor, la unión, las decisiones importantes, las elecciones. Te enfrentas a una encrucijada, y debes elegir tu camino con el corazón.",
      reversed:
        "La desarmonía, la separación, la indecisión, el conflicto. Te debates entre dos caminos, y el amor se ve empañado.",
    },
  },
  {
    name: "El Carro",
    image: "/tarot-images-bis/07-TheChariot.jpg",
    description: {
      upright:
        "El triunfo, la voluntad, el control, la dirección. Con determinación, avanzas hacia tus metas, superando obstáculos.",
      reversed:
        "La falta de control, la derrota, la falta de dirección, la falta de disciplina. Te desvías del camino, y pierdes el control de tu destino.",
    },
  },
  {
    name: "La Fuerza",
    image: "/tarot-images-bis/08-Strength.jpg",
    description: {
      upright:
        "La valentía, la compasión, el autocontrol, la fuerza interior. Domina tus miedos y pasiones, y encuentra la fuerza en tu interior.",
      reversed:
        "La debilidad, la falta de control, la agresividad, la inseguridad. Te dejas llevar por tus impulsos, y la fuerza se vuelve en tu contra.",
    },
  },
  {
    name: "El Ermitaño",
    image: "/tarot-images-bis/09-TheHermit.jpg",
    description: {
      upright:
        "La introspección, la soledad, la búsqueda interior, la sabiduría. Te retiras del mundo para buscar respuestas en tu interior.",
      reversed:
        "El aislamiento, la soledad no deseada, la oscuridad, el miedo a la introspección. Te alejas de los demás, pero no encuentras la paz.",
    },
  },
  {
    name: "La Rueda de la Fortuna",
    image: "/tarot-images-bis/10-WheelOfFortune.jpg",
    description: {
      upright:
        "El destino, el cambio, los ciclos, la buena fortuna. La rueda gira, y la vida te ofrece nuevas oportunidades.",
      reversed:
        "La mala suerte, la resistencia al cambio, la mala fortuna, la pérdida. Te aferras al pasado, y la rueda te arrastra hacia abajo.",
    },
  },
  {
    name: "La Justicia",
    image: "/tarot-images-bis/11-Justice.jpg",
    description: {
      upright:
        "El equilibrio, la verdad, la justicia, la responsabilidad. Cosechas lo que siembras, y la justicia se impone.",
      reversed:
        "La injusticia, el desequilibrio, la falta de responsabilidad, la parcialidad. La verdad se distorsiona, y la balanza se inclina.",
    },
  },
  {
    name: "El Colgado",
    image: "/tarot-images-bis/12-TheHangedMan.jpg",
    description: {
      upright:
        "El sacrificio, la perspectiva, la renuncia, la paciencia. Te entregas a una situación, y aprendes a ver las cosas desde una nueva perspectiva.",
      reversed:
        "El egoísmo, la resistencia al sacrificio, la impaciencia, la inacción. Te aferras a lo que no te sirve, y te niegas a cambiar.",
    },
  },
  {
    name: "La Muerte",
    image: "/tarot-images-bis/13-Death.jpg",
    description: {
      upright:
        "La transformación, el fin de un ciclo, la renovación, el cambio. Deja ir lo viejo, y prepárate para un nuevo comienzo.",
      reversed:
        "La resistencia al cambio, el estancamiento, el miedo a la muerte, la depresión. Te aferras al pasado, y te niegas a avanzar.",
    },
  },
  {
    name: "La Templanza",
    image: "/tarot-images-bis/14-Temperance.jpg",
    description: {
      upright:
        "El equilibrio, la armonía, la moderación, la paciencia. Encuentras la paz en la moderación, y te adaptas a las circunstancias.",
      reversed:
        "El desequilibrio, la impaciencia, la falta de armonía, el exceso. Te excedes en todo, y pierdes el equilibrio.",
    },
  },
  {
    name: "El Diablo",
    image: "/tarot-images-bis/15-TheDevil.jpg",
    description: {
      upright:
        "La atadura, la adicción, la sombra, la esclavitud. Estás atrapado en tus miedos y deseos, y te esclavizas a ellos.",
      reversed:
        "La liberación, la ruptura de ataduras, la superación de la sombra, la independencia. Te liberas de tus miedos, y recuperas tu libertad.",
    },
  },
  {
    name: "La Torre",
    image: "/tarot-images-bis/16-TheTower.jpg",
    description: {
      upright:
        "La destrucción, el caos, el cambio repentino, la revelación. La estructura se derrumba, y te enfrentas a una crisis.",
      reversed:
        "La resistencia al cambio, el miedo al caos, la evitación de la crisis, la posposición. Te aferras a lo conocido, y evitas el inevitable cambio.",
    },
  },
  {
    name: "La Estrella",
    image: "/tarot-images-bis/17-TheStar.jpg",
    description: {
      upright:
        "La esperanza, la inspiración, la fe, la curación. La luz de la esperanza te guía, y te conectas con tu propósito.",
      reversed:
        "La desesperanza, la falta de inspiración, la duda, la falta de fe. Te alejas de tu propósito, y te pierdes en la oscuridad.",
    },
  },
  {
    name: "La Luna",
    image: "/tarot-images-bis/18-TheMoon.jpg",
    description: {
      upright:
        "La intuición, la ilusión, la incertidumbre, el miedo. Te enfrentas a tus miedos y a la oscuridad de tu subconsciente.",
      reversed:
        "La claridad, la verdad revelada, la superación del miedo, la liberación de la ilusión. La oscuridad se disipa, y la verdad sale a la luz.",
    },
  },
  {
    name: "El Sol",
    image: "/tarot-images-bis/19-TheSun.jpg",
    description: {
      upright: "La alegría, el éxito, la vitalidad, la claridad. La luz del sol te ilumina, y la felicidad te inunda.",
      reversed:
        "La tristeza, la falta de alegría, la falta de energía, la confusión. La alegría se desvanece, y la oscuridad te envuelve.",
    },
  },
  {
    name: "El Juicio",
    image: "/tarot-images-bis/20-Judgement.jpg",
    description: {
      upright:
        "La resurrección, el despertar, la evaluación, el juicio final. Te enfrentas a tus acciones, y te preparas para un nuevo comienzo.",
      reversed:
        "La duda, la autocrítica, la falta de perdón, la indecisión. Te juzgas a ti mismo, y te niegas a avanzar.",
    },
  },
  {
    name: "El Mundo",
    image: "/tarot-images-bis/21-TheWorld.jpg",
    description: {
      upright: "La realización, la plenitud, la integración, el éxito. Alcanzas tus metas, y te sientes completo.",
      reversed:
        "La falta de realización, la incompletitud, la falta de integración, el fracaso. No encuentras la satisfacción, y te sientes incompleto.",
    },
  },
  
  // Arcanos Menores - Copas
  {
    name: "As de Copas",
    image: "/tarot-images-bis/Cups01.jpg",
    description: { upright: "Un nuevo comienzo emocional, oportunidades del corazón", reversed: "Rechazo emocional, oportunidades perdidas" },
  },
  {
    name: "Dos de Copas",
    image: "/tarot-images-bis/Cups02.jpg",
    description: { upright: "Unión y armonía en las relaciones", reversed: "Separación o desacuerdo" },
  },
  {
    name: "Tres de Copas",
    image: "/tarot-images-bis/Cups03.jpg",
    description: { upright: "Celebración, alegría y comunidad", reversed: "Retraso o reunión cancelada" },
  },
  {
    name: "Cuatro de Copas",
    image: "/tarot-images-bis/Cups04.jpg",
    description: { upright: "Contemplación, oportunidad a la vista", reversed: "Aceptación, movimiento hacia adelante" },
  },
  {
    name: "Cinco de Copas",
    image: "/tarot-images-bis/Cups05.jpg",
    description: { upright: "Pérdida, decepción, duelo", reversed: "Recuperación, aceptación del cambio" },
  },
  {
    name: "Seis de Copas",
    image: "/tarot-images-bis/Cups06.jpg",
    description: { upright: "Nostalgia, familia, buenos recuerdos", reversed: "Apegos al pasado, falta de progreso" },
  },
  {
    name: "Siete de Copas",
    image: "/tarot-images-bis/Cups07.jpg",
    description: { upright: "Ilusión, sueños y opciones", reversed: "Claridad, elección correcta" },
  },
  {
    name: "Ocho de Copas",
    image: "/tarot-images-bis/Cups08.jpg",
    description: { upright: "Alejarse en busca de algo mejor", reversed: "Apego, miedo al cambio" },
  },
  {
    name: "Nueve de Copas",
    image: "/tarot-images-bis/Cups09.jpg",
    description: { upright: "Deseos cumplidos, satisfacción", reversed: "Insatisfacción, falta de gratitud" },
  },
  {
    name: "Diez de Copas",
    image: "/tarot-images-bis/Cups10.jpg",
    description: { upright: "Felicidad familiar, armonía", reversed: "Ruptura, conflicto familiar" },
  },
  {
    name: "Sota de Copas",
    image: "/tarot-images-bis/Cups11.jpg",
    description: { upright: "Creatividad, joven sensible, buenos sentimientos", reversed: "Manipulación, torpeza emocional" },
  },
  {
    name: "Caballo de Copas",
    image: "/tarot-images-bis/Cups12.jpg",
    description: { upright: "Propuestas, cortejo, romancetismo", reversed: "Falso, insinceridad" },
  },
  {
    name: "Reina de Copas",
    image: "/tarot-images-bis/Cups13.jpg",
    description: { upright: "Mujer sensible, empatía, intuición", reversed: "Inseguridad, hipersensibilidad" },
  },
  {
    name: "Rey de Copas",
    image: "/tarot-images-bis/Cups14.jpg",
    description: { upright: "Hombre compasivo, diplomacia, generosidad", reversed: "Manipulación emocional, debilidad" },
  },
  
  // Arcanos Menores - Oros/Pentáculos
  {
    name: "As de Oros",
    image: "/tarot-images-bis/Pentacles01.jpg",
    description: { upright: "Oportunidad financiera, prosperidad", reversed: "Pérdida, desperdicio" },
  },
  {
    name: "Dos de Oros",
    image: "/tarot-images-bis/Pentacles02.jpg",
    description: { upright: "Balance, adaptabilidad en finanzas", reversed: "Desequilibrio, mal manejo" },
  },
  {
    name: "Tres de Oros",
    image: "/tarot-images-bis/Pentacles03.jpg",
    description: { upright: "Trabajo en equipo, maestría, aprendizaje", reversed: "Falta de colaboración, inexperiencia" },
  },
  {
    name: "Cuatro de Oros",
    image: "/tarot-images-bis/Pentacles04.jpg",
    description: { upright: "Seguridad, conservación, control", reversed: "Avaricia, falta de confianza" },
  },
  {
    name: "Cinco de Oros",
    image: "/tarot-images-bis/Pentacles05.jpg",
    description: { upright: "Dificultad financiera, estar dejado", reversed: "Recuperación, ayuda a camino" },
  },
  {
    name: "Seis de Oros",
    image: "/tarot-images-bis/Pentacles06.jpg",
    description: { upright: "Generosidad, dar y recibir", reversed: "Avaricia, deuda" },
  },
  {
    name: "Siete de Oros",
    image: "/tarot-images-bis/Pentacles07.jpg",
    description: { upright: "Esfuerzo, paciencia, inversión", reversed: "Falta de progreso, duda" },
  },
  {
    name: "Ocho de Oros",
    image: "/tarot-images-bis/Pentacles08.jpg",
    description: { upright: "Habilidad, dedicación, aprendizaje profesional", reversed: "Falta de habilidad, mala calidad" },
  },
  {
    name: "Nueve de Oros",
    image: "/tarot-images-bis/Pentacles09.jpg",
    description: { upright: "Lujo, independencia financiera", reversed: "Pérdida de independencia" },
  },
  {
    name: "Diez de Oros",
    image: "/tarot-images-bis/Pentacles10.jpg",
    description: { upright: "Riqueza, herencia, familia próspera", reversed: "Pérdida familiar, falta de estabilidad" },
  },
  {
    name: "Sota de Oros",
    image: "/tarot-images-bis/Pentacles11.jpg",
    description: { upright: "Joven ambicioso, estudiante dedicado", reversed: "Irresponsabilidad, pereza" },
  },
  {
    name: "Caballo de Oros",
    image: "/tarot-images-bis/Pentacles12.jpg",
    description: { upright: "Trabajo constante, confiabilidad, prosperidad", reversed: "Vagancia, falta de motivación" },
  },
  {
    name: "Reina de Oros",
    image: "/tarot-images-bis/Pentacles13.jpg",
    description: { upright: "Mujer práctica, generosa, amable", reversed: "Codicia, pérdida financiera" },
  },
  {
    name: "Rey de Oros",
    image: "/tarot-images-bis/Pentacles14.jpg",
    description: { upright: "Hombre rico, líder seguro, abundancia", reversed: "Avaricia, corrupción" },
  },
  
  // Arcanos Menores - Espadas
  {
    name: "As de Espadas",
    image: "/tarot-images-bis/Swords01.jpg",
    description: { upright: "Claridad mental, verdad, nuevo comienzo", reversed: "Confusión, mentira" },
  },
  {
    name: "Dos de Espadas",
    image: "/tarot-images-bis/Swords02.jpg",
    description: { upright: "Indecisión, punto muerto, encrucijada", reversed: "Claridad, resolución" },
  },
  {
    name: "Tres de Espadas",
    image: "/tarot-images-bis/Swords03.jpg",
    description: { upright: "Dolor, pena, separación", reversed: "Curación, reconciliación" },
  },
  {
    name: "Cuatro de Espadas",
    image: "/tarot-images-bis/Swords04.jpg",
    description: { upright: "Descanso, recuperación, meditación", reversed: "Regreso, movimiento" },
  },
  {
    name: "Cinco de Espadas",
    image: "/tarot-images-bis/Swords05.jpg",
    description: { upright: "Conflicto, derrota, tensión", reversed: "Reconciliación, paz" },
  },
  {
    name: "Seis de Espadas",
    image: "/tarot-images-bis/Swords06.jpg",
    description: { upright: "Viaje, transición, mudanza", reversed: "Estancamiento, bloqueo" },
  },
  {
    name: "Siete de Espadas",
    image: "/tarot-images-bis/Swords07.jpg",
    description: { upright: "Estrategia, engaño, huida", reversed: "Sinceridad, salida honesta" },
  },
  {
    name: "Ocho de Espadas",
    image: "/tarot-images-bis/Swords08.jpg",
    description: { upright: "Limitación, prisión mental, miedo", reversed: "Liberación, claridad" },
  },
  {
    name: "Nueve de Espadas",
    image: "/tarot-images-bis/Swords09.jpg",
    description: { upright: "Ansiedad, pesadillas, estrés", reversed: "Paz mental, alivio" },
  },
  {
    name: "Diez de Espadas",
    image: "/tarot-images-bis/Swords10.jpg",
    description: { upright: "Fin, traición, punto de quiebre", reversed: "Recuperación, nuevo comienzo" },
  },
  {
    name: "Sota de Espadas",
    image: "/tarot-images-bis/Swords11.jpg",
    description: { upright: "Joven hablador, lógico, comunicativo", reversed: "Malicia, mala comunicación" },
  },
  {
    name: "Caballo de Espadas",
    image: "/tarot-images-bis/Swords12.jpg",
    description: { upright: "Aventura, búsqueda, cambio", reversed: "Impulsividad, conflicto" },
  },
  {
    name: "Reina de Espadas",
    image: "/tarot-images-bis/Swords13.jpg",
    description: { upright: "Mujer independiente, inteligente, viuda", reversed: "Crueldad, frialdad" },
  },
  {
    name: "Rey de Espadas",
    image: "/tarot-images-bis/Swords14.jpg",
    description: { upright: "Hombre lógico, inteligencia, poder mental", reversed: "Tiranía, crueldad intelectual" },
  },
  
  // Arcanos Menores - Bastos
  {
    name: "As de Bastos",
    image: "/tarot-images-bis/Wands01.jpg",
    description: { upright: "Inspiración, nuevo proyecto, energía", reversed: "Bloqueo, falta de energía" },
  },
  {
    name: "Dos de Bastos",
    image: "/tarot-images-bis/Wands02.jpg",
    description: { upright: "Planificación, decisión, poder", reversed: "Cambio de planes, inseguridad" },
  },
  {
    name: "Tres de Bastos",
    image: "/tarot-images-bis/Wands03.jpg",
    description: { upright: "Trabajo en progreso, colaboración, visión", reversed: "Retraso, falta de coordinación" },
  },
  {
    name: "Cuatro de Bastos",
    image: "/tarot-images-bis/Wands04.jpg",
    description: { upright: "Celebración, armonía, éxito", reversed: "Privación, ruptura" },
  },
  {
    name: "Cinco de Bastos",
    image: "/tarot-images-bis/Wands05.jpg",
    description: { upright: "Conflicto, competencia, desacuerdo", reversed: "Resolución, acuerdo" },
  },
  {
    name: "Seis de Bastos",
    image: "/tarot-images-bis/Wands06.jpg",
    description: { upright: "Victoria, éxito, reconocimiento", reversed: "Falta de reconocimiento, fracaso" },
  },
  {
    name: "Siete de Bastos",
    image: "/tarot-images-bis/Wands07.jpg",
    description: { upright: "Desafío, defensa, valor", reversed: "Claudicación, derrota" },
  },
  {
    name: "Ocho de Bastos",
    image: "/tarot-images-bis/Wands08.jpg",
    description: { upright: "Movimiento rápido, progreso, velocidad", reversed: "Retrasos, indecisión" },
  },
  {
    name: "Nueve de Bastos",
    image: "/tarot-images-bis/Wands09.jpg",
    description: { upright: "Resistencia, fuerza, persistencia", reversed: "Agotamiento, falta de energía" },
  },
  {
    name: "Diez de Bastos",
    image: "/tarot-images-bis/Wands10.jpg",
    description: { upright: "Carga, responsabilidad, presión", reversed: "Alivio, liberación" },
  },
  {
    name: "Sota de Bastos",
    image: "/tarot-images-bis/Wands11.jpg",
    description: { upright: "Joven entusiasta, aventurero, pasionario", reversed: "Falta de dirección, apatía" },
  },
  {
    name: "Caballo de Bastos",
    image: "/tarot-images-bis/Wands12.jpg",
    description: { upright: "Pasión, energía, aventura", reversed: "Falta de control, impulsividad" },
  },
  {
    name: "Reina de Bastos",
    image: "/tarot-images-bis/Wands13.jpg",
    description: { upright: "Mujer confiada, apasionada, magnética", reversed: "Celos, inseguridad" },
  },
  {
    name: "Rey de Bastos",
    image: "/tarot-images-bis/Wands14.jpg",
    description: { upright: "Hombre carismático, visionario, líder", reversed: "Tiranía, arrogancia" },
  },
]
